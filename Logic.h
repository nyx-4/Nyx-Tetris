...
void Putch(char ch) { write(STDIN_FILENO, &ch, 1); }
...
// @param Taps: A bitmask, can be random but recommended is to copy from
...
short ModernRotate(short block, short RandomNumber = 0) {
    switch (RotationAlgorithm) {
        ...
        case 4:
            for (int i = 0; i < RandomNumber; i++) block = Rotate(block);
            return block;
...
bool isCollision(short block, int pos_y, int pos_x) {
    if (pos_x + EmptyColumnsOnLeft(block) < 0 ||
        pos_x - EmptyColumnsOnRight(block) > 12)
        return true;
    short curPartOfGame = 0;
    for (int i = 0; i < 4; i++)
        if (pos_y + i < 0 || pos_y + i > Height)
            curPartOfGame = (curPartOfGame << 4);
        else if (12 - pos_x < 0) {
            curPartOfGame = ((curPartOfGame << 4) |
                             ((GameArr[pos_y + i] << (pos_x - 12)) & 0b1111));
        } else
            curPartOfGame = ((curPartOfGame << 4) |
                             ((GameArr[pos_y + i] >> (12 - pos_x)) & 0b1111));
    ...
    for (int i = 0; i < 4; i++)
        if (pos_y + i >= 0 && pos_y + i < Height)
            if (12 - pos_x < 0)
                GameArr[pos_y + i] =
                    GameArr[pos_y + i] |
                    (((block >> (12 - 4 * i)) & 0b1111) >> (pos_x - 12));
            else
                GameArr[pos_y + i] =
                    GameArr[pos_y + i] |
                    (((block >> (12 - 4 * i)) & 0b1111) << (12 - pos_x));
}
...
void StartGame() {
    short block = 0b0100010001000100, blockNext = 0b0100010001000100, pos_y = 0,
          pos_x = 8, RotateState = 0, TicksCounter = 3, tempRandom = 1;
    ...
    else if (isCustomPolyomino)
        blockNext =
            CustomPolyominos[Rand(7, Garbage) % NumberOfCustomPolyominos];
    else if (isTrueRandom)
        blockNext = Rand(16, Garbage);
    else if (isDenseRandom)
        blockNext = Rand(16, Garbage) | Rand(16, Garbage);
    ...
    if (isClassic) block = ClassicTetrominos[Rand(7, Garbage) % 7];
        ...
        block = CustomPolyominos[Rand(7, Garbage) % NumberOfCustomPolyominos];
    else if (isTrueRandom)
        block = Rand(16, Garbage);
    else if (isDenseRandom)
        block = Rand(16, Garbage) | Rand(16, Garbage);
    else if (isHollowRandom) {
        blockNext = 0;
        while (blockNext == 0)
            blockNext = Rand(16, Garbage) & Rand(16, Garbage);
        ...
        switch (ch) {
            ...
            case 'D':
                if (!isCollision(block, pos_y, pos_x - 1)) {
                    ...
                    pos_x--;
                    DrawBlock(block, pos_y, pos_x,
                              BlockColor(block, RotateState));
                }
                break;
            case 'j':
            case 's':
            ...
            case 'A':
                ...
                if (!isCollision(ModernRotate(block, tempRandom), pos_y,
                                 pos_x)) {
            ...
            case 'p':
                while (ch != 'r' && ch != 'R' && ch != 'q' && ch != 'Q')
                    ch = Getch();
                break;
            default: TicksCounter--; break;
        }

        if (TicksCounter < 0) {
            TicksCounter += Speed;
            if (!isCollision(block, pos_y + 1,
                             pos_x)) { // Can go down, then go down..
                DrawBlock(block, pos_y, pos_x, "\033[49m  ");
                pos_y++;
                ...
                if (isClassic)
                    blockNext = ClassicTetrominos[Rand(7, Garbage) % 7];
                else if (isCustomPolyomino)
                    blockNext = CustomPolyominos[Rand(7, Garbage) %
                                                 NumberOfCustomPolyominos];
                else if (isTrueRandom)
                    blockNext = Rand(16, Garbage);
                else if (isDenseRandom)
                    blockNext = Rand(16, Garbage) | Rand(16, Garbage);
                ...
                DrawBlock(block, pos_y, pos_x,
                          BlockColor(block, RotateState)); //"\033[41m \033[49m"
            }
        }
